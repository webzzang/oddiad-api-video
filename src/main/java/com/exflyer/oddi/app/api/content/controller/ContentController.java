...
public class ContentController {
   ...
   * 메인광고
   *
   * @param paramMap (device_id)디바이스 생성시 받은 ID 값 (device_latitude)디바이스에서 수집된 위치 정보 (위도) 값이 없으면 "" 로 전달처리
...
//		paramMap.put("device_id", "testid");
    paramMap
      .put("device_latitude", CryptoUtil.getDecMsg(contentGetMainReq.getDevice_latitude())); //디바이스에서 수집된 위치 정보 (위도)
    paramMap
      .put("device_longitude", CryptoUtil.getDecMsg(contentGetMainReq.getDevice_longitude())); //디바이스에서 수집된 위치 정보 (경도)
...
//					as content_ext
   ...
   * 사이드 광고
   *
   * @param paramMap (device_id)디바이스 생성시 받은 ID 값
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
  ...
  public ApiResWithSingleData contentSide(@Validated @RequestBody ContentGetSubReq contentGetSubReq)
    ...
    try {
      ...
      if (deviceInfo.get("side_contents_type").equals("weather")) {
        setSideWeather(adList);
      } else {
        setSideBanner(list, adList);
      }
  ...
  private void setSideBanner(List<Map<String, Object>> list, List<Map<String, Object>> adList) {
    for (Map<String, Object> inMap : list) {
      Map<String, Object> outMap = new HashMap<String, Object>();
      outMap.put("content_id", inMap.get("content_id"));
      outMap.put("content_type", inMap.get("content_type"));
      outMap.put("content_duration", inMap.get("content_duration"));
      outMap.put("content_url", Arrays.asList(String.valueOf(inMap.get("content_url"))));
      adList.add(outMap);
    }
  }

  private void setSideWeather(List<Map<String, Object>> adList) {
   ...
   * 컨텐츠 재생 상태
   *
   * @param paramMap (device_id) 디바이스 생성시 받은 ID 값 (content_id) 컨텐츠 ID (content_state) 컨텐츠 상태 값 | start -> 컨텐츠의 재생 시작을 알림
   * / end -> 컨텐츠 재생의 끝을 알림 (content_type) 컨텐츠 타입 값 | image -> 이미지 광고 컨텐츠  / video -> 동영상 광고 컨텐츠
   * (content_screen_position) 재생중인 광고 영역 값 Ex) "screen_pos_main" -> 메인화면 광고중 "screen_pos_side" ->  우측화면 광고중
   * "screen_pos_bottom" ->  하단영역 광고중 (content_screen_type) 재생중인 광고 영역 타입 Ex) "divisions_1" -> 메인광고 full 화면
   * "divisions_2" -> 메인광고 + 오른쪽 배너 광고 화면 "divisions_3" -> 메인광고 + 오른쪽 배너 + 하단 배너 광고 화면
   */
